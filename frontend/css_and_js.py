...
js_part_getindex_txt2img_history="""
const root = document.querySelector('gradio-app').shadowRoot;
const getIndex = function(){
const selected = root.querySelector('#txt2img_gallery_output_history .\\\\!ring-2');
alert(selected);
return selected ? [...selected.parentNode.children].indexOf(selected) : 0;
};"""
js_copy_selected_txt2img_history = "async (x) => {" + js_part_getindex_txt2img_history + js_part_copy_to_clipboard
js_return_selected_txt2img_history = "(x) => {" + js_part_getindex_txt2img_history + js_part_clear_img2img + """
return [x[getIndex()].replace('data:;','data:image/png;')];
}"""
js_save_selected_image_to_csv = "(x) => {" + js_part_getindex_txt2img + """
return [x[getIndex()].replace('data:;','data:image/png;'),getIndex()];
}"""
js_save_selected_image_to_csv_history = "(x) => {" + js_part_getindex_txt2img_history  + """
return [x[getIndex()].replace('data:;','data:image/png;'),getIndex()];
}"""
